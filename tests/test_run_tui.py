...
@unittest.skipUnless(HAS_TEXTUAL, "textual not installed")
class TestRunTuiSkillActions(unittest.TestCase):
    """Tests for skill_add, skill_edit, skill_remove (lines 1618-1651)."""
    ...
    def test_skill_add_success(self):
        """run_tui: skill_add → cmd_skill_add called → sync skill-add."""
        result, mock_cmd, mock_sync = self._run_action("skill_add", None, "cmd_skill_add")
        mock_cmd.assert_called_once()
        mock_sync.assert_any_call("skill-add")

    def test_skill_add_keyboard_interrupt(self):
        """run_tui: skill_add → KeyboardInterrupt → sync still called (line 1624-1625)."""
        result, _, mock_sync = self._run_action("skill_add", None, "cmd_skill_add", exc=KeyboardInterrupt)
        mock_sync.assert_any_call("skill-add")

    def test_skill_add_exception(self):
        """run_tui: skill_add → Exception → sync still called (line 1626-1627)."""
        result, _, mock_sync = self._run_action("skill_add", None, "cmd_skill_add", exc=RuntimeError("err"))
        mock_sync.assert_any_call("skill-add")

    def test_skill_edit_success(self):
        """run_tui: skill_edit → cmd_skill_edit called → sync skill-edit."""
        result, mock_cmd, mock_sync = self._run_action("skill_edit", "myskill", "cmd_skill_edit")
        mock_cmd.assert_called_once_with("myskill")
        mock_sync.assert_any_call("skill-edit")

    def test_skill_edit_keyboard_interrupt(self):
        """run_tui: skill_edit → KeyboardInterrupt → sync still called (line 1635-1636)."""
        result, _, mock_sync = self._run_action("skill_edit", "myskill", "cmd_skill_edit", exc=KeyboardInterrupt)
        mock_sync.assert_any_call("skill-edit")

    def test_skill_edit_exception(self):
        """run_tui: skill_edit → Exception → sync still called (line 1637-1638)."""
        result, _, mock_sync = self._run_action("skill_edit", "myskill", "cmd_skill_edit", exc=OSError("err"))
        mock_sync.assert_any_call("skill-edit")

    def test_skill_remove_success(self):
        """run_tui: skill_remove → cmd_skill_remove called → sync skill-remove."""
        result, mock_cmd, mock_sync = self._run_action("skill_remove", "myskill", "cmd_skill_remove")
        mock_cmd.assert_called_once_with("myskill")
        mock_sync.assert_any_call("skill-remove")

    def test_skill_remove_keyboard_interrupt(self):
        """run_tui: skill_remove → KeyboardInterrupt → sync still called (line 1645-1646)."""
        result, _, mock_sync = self._run_action("skill_remove", "myskill", "cmd_skill_remove", exc=KeyboardInterrupt)
        mock_sync.assert_any_call("skill-remove")

    def test_skill_remove_exception(self):
        """run_tui: skill_remove → Exception → sync still called (line 1648-1649)."""
        result, _, mock_sync = self._run_action("skill_remove", "myskill", "cmd_skill_remove", exc=ValueError("err"))
        mock_sync.assert_any_call("skill-remove")
